...
def stream():
    ...
    return Response(
        input_parser.generate_words(file_path, live), mimetype="text/event-stream"
    )


@app.route("/get-saved-transcript-keywords")
def get_saved_transcript_keywords():
    return input_parser.get_matches()
...
def saved_text_route():
    ...
    return input_parser.get_raw_transcript()
...
def send_person():
    try:
        ...
        person_name = request.args.get(
            "name"
        )  # or you can use request.json if it's a POST with a body
        ...
        person_data = next(
            (person for person in data if person.get("name") == person_name), None
        )
