...
function Glossary({ selectedWord, position, setSelectedWord }) {
  ...
  useEffect(() => {
    const fetchDefinition = async () => {
      ...
      try {
        ...
        const response = await axios.get(`http://localhost:5000/api/get_definition_api/${lowercasedWord}`);
  ...
  const handleShowMore = async () => {
    ...
    if (showMore) {
      ...
      try {
        const response = await axios.get(`http://localhost:5000/api/get_gpt_response/${selectedWord}`);
        ...
        console.error("Error fetching extended definition:", error.response ? error.response.data.message : error.message);
        setExtendedDefinition('No additional info available');
  ...
  return (
    ...
    >
      ...
      <p>{definition}</p>
      {showMore && <div className="extended-definition" dangerouslySetInnerHTML={{ __html: extendedDefinition }} />}
